...
void FEdGraphFormatter::SimpleRelativeFormatting()
{
	...
	const int DeltaX = FMath::CeilToInt(NodeToKeepStill->NodePosX - PreviousNodeToKeepStillPosition.X);
	const int DeltaY = FMath::CeilToInt(NodeToKeepStill->NodePosY - PreviousNodeToKeepStillPosition.Y);
		...
		for (UEdGraphNode* Param : ParameterNodes)
		{
			if (ParameterParentMap.Contains(Param))
			{
				...
				if (!bIsChild && !ParamFormatter->IsUsingHelixing())
				{
					...
					const int32 Delta = FMath::CeilToInt(ParamNodeBounds.Right + PinPadding.X - Node->NodePosX);
					if (Delta > 0)
					{
...
int32 FEdGraphFormatter::GetChildX(UEdGraphNode* Parent, UEdGraphNode* Child, EEdGraphPinDirection Direction, bool bUseClusterNodes)
{
	...
	// UE_LOG(LogBlueprintAssist, VeryVerbose, TEXT("Node %s New Node Pos %d | %s | %s | %s"), *Link.ToString(), FMath::CeilToInt(NewNodePos), *ParentBounds.ToString(), *ChildBounds.ToString(), *LargerBounds.ToString());

	return FMath::CeilToInt(NewNodePos);
}
