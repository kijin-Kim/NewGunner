...
UNexusAnimMontagePlayerComponent::UNexusAnimMontagePlayerComponent()
{
	PrimaryComponentTick.bCanEverTick = true;
	SetIsReplicatedByDefault(true);
}

void UNexusAnimMontagePlayerComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);
	DOREPLIFETIME_CONDITION(UNexusAnimMontagePlayerComponent, ReplicatedAnimMontageData, COND_SimulatedOnly);
}

void UNexusAnimMontagePlayerComponent::BeginPlay()
{
	Super::BeginPlay();
	checkf(GetOwner()->Implements<UNexusAnimMontagePlayerInterface>(), TEXT("Owner Must Implements AnimMontagePlayerInterface"));
}

void UNexusAnimMontagePlayerComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
float UNexusAnimMontagePlayerComponent::PlayMontage(UAnimMontage* AnimMontage, bool bIsThirdPerson, float InPlayRate, FName StartSectionName)
{
...
void UNexusAnimMontagePlayerComponent::StopMontage(UAnimMontage* AnimMontage, bool bIsThirdPerson)
{
...
FOnMontageBlendingOutStarted* UNexusAnimMontagePlayerComponent::GetMontageBlendingOutStartedDelegate(UAnimMontage* AnimMontage, bool bIsThirdPerson)
{
	UAnimInstance* AnimInstance = GetDesiredAnimInstance(bIsThirdPerson);
	return AnimInstance ? AnimInstance->Montage_GetBlendingOutDelegate(AnimMontage) : nullptr;
}

UAnimInstance* UNexusAnimMontagePlayerComponent::GetDesiredAnimInstance(bool bIsThirdPerson) const
{
	USkeletalMeshComponent* TargetMesh = bIsThirdPerson
		                                     ? INexusAnimMontagePlayerInterface::Execute_GetThirdPersonMeshComponent(GetOwner())
		                                     : INexusAnimMontagePlayerInterface::Execute_GetFirstPersonMeshComponent(GetOwner());
	return (TargetMesh) ? TargetMesh->GetAnimInstance() : nullptr;
}

void UNexusAnimMontagePlayerComponent::AuthUpdateReplicatedAnimMontage()
{
	USkeletalMeshComponent* ThirdPersonMeshComponent = INexusAnimMontagePlayerInterface::Execute_GetThirdPersonMeshComponent(GetOwner());
...
float UNexusAnimMontagePlayerComponent::LocalPlayMontage(UAnimMontage* AnimMontage, bool bIsThirdPerson, float InStartTime, float InPlayRate, FName StartSectionName)
{
	UAnimInstance* AnimInstance = GetDesiredAnimInstance(bIsThirdPerson);
...
void UNexusAnimMontagePlayerComponent::OnRep_ReplicatedAnimMontage()
{
